...
function wordTyped(word){
    var new_word = document.getElementById("chatMessage").value.toLowerCase();
...
function isMispelled(word){
    word = word.toLowerCase();
    rList = find_similar(word, lower_thresh)[0];

    if(rList.length > 0){
        for(var i = 0; i < rList.length && i < max_num_of_items; i++){
            return !(i == 0 && rList[0].toLowerCase() == word);
        }
    } else {
        return true;
    }

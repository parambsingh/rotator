...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        this.errorLog = $("#status"); //Writing errors
        this.responses = [];
        ...
        function noserializeAndUploadFiles(s, obj, files) {
            ...
            for (var i = 0; i < files.length; i++) {
                if (!isFileTypeAllowed(obj, s, files[i].name)) {
                    if (s.showError) $("<div><span color='red'><b>" + files[i].name + "</b> " + s.extErrorStr + s.allowedTypes + "</span></div>").appendTo(obj.errorLog);
                    continue;
                }
                if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if (s.showError) $("<div><span color='red'><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</span></div>").appendTo(obj.errorLog);
                    continue;
                }
        ...
        function createCustomInputFile(obj, group, s, uploadLabel) {
            ...
            var fileUploadId = "ajax-upload-id" + s.uniqueIdStr;
